...
public static partial class PawnEditor
{
    ...
    public static void Paste(Pawn into)
    {
        ...
        if (into.inventory != null && clipboard?.inventory != null)
        {
            ...
            // FIX #008: Snapshot the list to avoid modifying collection during iteration.
    ...
    private static T Clone<T>(this T thing) where T : Thing
    {
        ...
        // FIX #008: Copy stackCount so stacked items (meals, medicine) keep their count.
        clone.stackCount = thing.stackCount;

        // FIX #008: Copy quality so masterwork/legendary items don't degrade to normal.
        ...
        // FIX #008: Copy color so dyed apparel retains its tint.
