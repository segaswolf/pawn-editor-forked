...
public static class StartingThingsManager
{
    ...
    public static void ProcessScenario()
    {
        ...
        // FIX #009: Snapshot the parts list before iterating.
        // Original code iterated Find.Scenario.AllParts while calling RemovePart() inside the loop,
        // which can skip items or throw. If anything fails, RestoreScenario() recovers the originals.
        ...
        {
            Log.Error($"[PawnEditor] ProcessScenario failed — restoring original parts: {ex.Message}");
            RestoreScenario();
            return;
        }
