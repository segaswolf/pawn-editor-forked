...
public partial class TabWorker_Bio_Humanlike
{
    private void DoLeft(Rect inRect, Pawn pawn)
    {
        ...
        if (Widgets.ButtonText(colorRect, "PawnEditor.PickColor".Translate()))
        {
            ...
            // FIX #002: Don't mutate the shared ColorDef.color — find the nearest matching ColorDef instead.
            Find.WindowStack.Add(new Dialog_ColorPicker(color =>
            {
                var best = DefDatabase<ColorDef>.AllDefsListForReading
                    .Where(cd => cd != null)
                    .OrderBy(cd => ColorDistance(cd.color, color))
                    .FirstOrDefault();
                if (best != null)
                    pawn.story.favoriteColor = best;
    ...
    // FIX #002 helper: Euclidean distance in RGB space between two colors.
