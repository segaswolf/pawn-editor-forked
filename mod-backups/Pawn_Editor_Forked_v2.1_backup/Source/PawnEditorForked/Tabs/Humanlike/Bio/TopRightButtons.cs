...
public partial class TabWorker_Bio_Humanlike
{
    ...
    public static void SetDevStage(Pawn pawn, DevelopmentalStage stage)
    {
        ...
        if (oldStage != stage)
        {
            ...
            // Without this, pawn.story.Adulthood stays null and the editor can't display it.
            if (stage == DevelopmentalStage.Adult && pawn.story.Adulthood == null)
            {
                var validBackstories = DefDatabase<BackstoryDef>.AllDefsListForReading
                    .Where(bs => bs.slot == BackstorySlot.Adulthood)
                    .ToList();
                if (validBackstories.Any())
                    pawn.story.Adulthood = validBackstories.RandomElement();
            }
            // Clear adulthood backstory when going back to child.
    ...
    // FIX #010: Added null checks and snapshot lists to prevent NRE with custom xenotypes.
    // Original crashed when: RemoveGene(null) from FirstOrDefault, pawn.genes null (no Biotech),
    // or iterating gene list while removing from it.
    private static void ClearXenotype(Pawn pawn)
    {
        ...
        {
            var genesToRemove = pawn.genes.xenotype.genes.ToList();
            foreach (var xenotypeGene in genesToRemove)
            {
                var geneList = pawn.genes.xenotype.inheritable ? pawn.genes.Endogenes : pawn.genes.Xenogenes;
                var gene = geneList?.FirstOrDefault(g => g.def == xenotypeGene);
        ...
        {
            var genesToRemove = customXenotype.genes.ToList();
            foreach (var xenotypeGene in genesToRemove)
            {
                var geneList = customXenotype.inheritable ? pawn.genes.Endogenes : pawn.genes.Xenogenes;
                var gene = geneList?.FirstOrDefault(g => g.def == xenotypeGene);
                if (gene != null) pawn.genes.RemoveGene(gene);
            }
        }
