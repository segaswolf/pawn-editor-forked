...
public static partial class PawnEditor
{
    ...
    private static void DoPawnList(Rect inRect, List<Pawn> pawns, List<string> sections, int sectionCount, Action<Pawn, int, int> onReorder)
    {
        ...
        for (var i = 0; i < pawns.Count; i++)
        {
            ...
            // Now draws a placeholder instead of nothing when the atlas is dropped, so the slot
            // reads as "loading" rather than a black gap.
            ...
            PawnEditor.DrawPortraitOrPlaceholder(
                new(105f - portraitSize.x / 2f, 40f - portraitSize.y / 2f, portraitSize.x, portraitSize.y), portraitTex);
